...
admin.initializeApp({
  ...
  databaseURL: 'https://alfred-cqletl.firebaseio.com'
});
...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  function other(agent){
  	...
  	agent.add('Sorry, We are not providing these services as of now.');
    agent.context.set({
                'name':'defaultwelocmeintent-followup',
                'lifespan': 5
                });
        ...
        const feinlength = fein.toString().length.toString();
            ...
            if(isNaN(fein)){
                ...
                agent.add('What is the nature of your Business ?');
              	...
              	return axios.post('http://calm-peak-72227.herokuapp.com/test/api/business_info/storedata/1',
                       {"fein": fein}).then((result) => {
                  		console.log(result.data);
                  		console.log('Store fein to api');
                  		agent.add('Store fein');
                ...
                if(fein1 == ""){
              	...
              	.then((result) => {
                	console.log(result.data);	
                  	...
                  	if(result.data.api_business_information.Name != 'None'){
                    agent.add('Here is what I found:');  
                   	agent.add('Name: '+ result.data.api_business_information.Name.toString());
                    agent.add('Address: '+ result.data.api_business_information.Address.toString() );
                    agent.add('Phone: '+ result.data.api_business_information.Phone.toString());
                    agent.add('Website: '+ result.data.api_business_information.Website.toString());
                    agent.add('Additional Business Operations: '+ result.data.api_business_information['Additional Business Operations'].toString());
                    ...
                    agent.add(new Suggestion('Edit'));
                    	...
                    	agent.add('I am not able to found any information.');
                      	agent.add('Please enter business name');
            ...
            "",
             ...
             "AlfredFlags": ""
              ...
              console.log(result.data);
            	...
            	agent.add('Okay. I have found an existing Workers Comp Quote for your business.');
                agent.add('Do you still want to continue?');
                agent.add(new Suggestion('Yes'));
                agent.add(new Suggestion('No'));
            }
  ...
  **/
    function editna(agent){
        ...
        agent.add('Ok, I have updated business name.  Do you want to edit Address?');
        agent.add(new Suggestion('Yes'));
        agent.add(new Suggestion('No')); 
  ...
  intentMap.set('askbussop',askbussop);
